...
void d2N_transverse_4060()
{
  ...
  Double_t minp=1;
  Double_t maxp=6;
  TH2D * d2N = new TH2D("d2N", "Avg. number density",20,minp,maxp,20,1,5);
  for(Int_t i = 0; i<entries; i++) //going through all ntrack entries
  {
    ...
    for(Int_t j = 0; j<ntrack; j++)
    {
        ...
        if(dphi<2*TMath::Pi()/3 && dphi>TMath::Pi()/3 &&abs(eta[j])<2.5)//selecting region and making eta cut
        {
            N++;
        }  
    }
...
//d2N->Draw("LEGO");
  ...
  final->Draw("HIST");
  c2->SaveAs("d2N_transverse_4060.pdf");
}