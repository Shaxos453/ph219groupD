...
void d2N_toward_4060()
{
  TFile *f = new TFile("13TeV_CR0_RHoff.root"); //opens file 13TeV_CR0_RHoff.root
  TTree *tree = (TTree*)f->Get("pytree4060"); //opens tree of multiplicity class 20-40
  ...
  TCanvas *c2 = new TCanvas("c2", "Average number density for toward region", 750, 400);

  //declaring a histogram with 125 bins, and X axis range from 0 to Pi
  TH2D * d2N = new TH2D("d2N", "Avg. number density",20,0,5,20,0,4);
  for(Int_t i = 0; i<entries; i++) //going through all ntrack entries
  {
    ...
    for(Int_t j = 0; j<ntrack; j++)
    {
        ...
        if(dphi<TMath::Pi()/3 && abs(eta[j])<2.5)//selecting region and making eta cut
        {
            N++;
        }  
    }
