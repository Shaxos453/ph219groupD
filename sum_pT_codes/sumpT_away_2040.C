void sumpT_away_2040()
{
  ...
  TCanvas *c2 = new TCanvas("c2", "Average scalar pT sum density for away region", 750, 400);
  ...
  for(Int_t i = 0; i<entries; i++) //going through all ntrack entries
  {
    ...
    for(Int_t j = 0; j<ntrack; j++)
    {
      if(pT[j]>0.5)
      {
        ...
        if(dphi<TMath::Pi() && dphi>2*TMath::Pi()/3 && abs(eta[j])<2.5)
        {
            sum_pT+=pT[j];
        }
      }
  ...
  final->SetTitle("Away region (20-40)");
