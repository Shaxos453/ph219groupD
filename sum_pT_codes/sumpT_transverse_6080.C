void sumpT_transverse_6080()
{
  TFile *f = new TFile("13TeV_CR0_RHoff.root"); //opens file 13TeV_CR0_RHoff.root
  TTree *tree = (TTree*)f->Get("pytree6080"); //opens tree of multiplicity class 0-20
  ...
  TCanvas *c2 = new TCanvas("c2", "Average scalar pT sum density for transverse region", 750, 400);

  //declaring a histogram with 125 bins, and X axis range from 0 to Pi
  TH2D * d2_sum_pT = new TH2D("d2N", "Avg. scalar pT sum density",20,0,5,20,0,4);
  for(Int_t i = 0; i<entries; i++) //going through all ntrack entries
  {
    ...
    for(Int_t j = 0; j<ntrack; j++)
    {
      if(pT[j]>0.5)
      {
        ...
        if(dphi<2*TMath::Pi()/3 && dphi>TMath::Pi()/3 && abs(eta[j])<2.5)
        {
            sum_pT+=pT[j];
        }
      }
  ...
  TProfile* final = d2_sum_pT->ProfileX();
  final->SetStats(0);
  Int_t check=0;
