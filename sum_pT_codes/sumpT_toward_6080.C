void sumpT_toward_6080()
{
  TFile *f = new TFile("13TeV_CR0_RHoff.root"); //opens file 13TeV_CR0_RHoff.root
  TTree *tree = (TTree*)f->Get("pytree6080"); //opens tree of multiplicity class 60-80
  ...
  TCanvas *c2 = new TCanvas("c2", "Average scalar pT sum density for toward region", 750, 400);
  ...
  for(Int_t i = 0; i<entries; i++) //going through all ntrack entries
  {
    ...
    for(Int_t j = 0; j<ntrack; j++)
    {
      if(pT[j]>0.5)
      {
        ...
        if(dphi<TMath::Pi()/3 && abs(eta[j])<2.5)
        {
            sum_pT+=pT[j];
        }
      }
  ...
  final->SetTitle("Toward region (60-80)");
